...
/** Uniforms passed to the laser shader; typed so we avoid `any` in refs. */
type LaserUniforms = {
    iTime: { value: number };
...
export function LaserFlow({
    ...
    useEffect(() => {
        ...
        const onPointer = (ev: PointerEvent) => updateMouse(ev.clientX, ev.clientY);
    ...
    return <div ref={mountRef} className={className ? `relative h-full w-full ${className}` : 'relative h-full w-full'} style={style} />;
}
