                ...
                <div className="relative flex h-full items-end border-b border-slate-200/80 pb-1.5 dark:border-slate-600/40">
                    <div className="pointer-events-none absolute inset-x-0 top-0 bottom-0 flex flex-col justify-between text-[10px] text-slate-400">
                        {ticks.map((tick, index) => (
                            <div key={tick} className="flex items-center gap-1">
                                ...
                                <div className={index === ticks.length - 1 ? 'h-px flex-1 bg-slate-300 dark:bg-slate-600' : 'h-px flex-1 bg-slate-200/60 dark:bg-slate-700/40'} />
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="pointer-events-none absolute top-0 right-0 bottom-0 left-8 flex justify-between">
                        {sortedTools.map((tool) => (
                            <div key={tool.name} className="h-full w-px bg-slate-200/30 dark:bg-slate-700/30" />
                        ))}
                    </div>
