...
export function MostBorrowedBarChart({ tools }: MostBorrowedBarChartProps) {
    ...
    const max = tools.reduce((accumulator, tool) => Math.max(accumulator, tool.count), 0) || 1;

    if (!tools.length) {
        return (
            <section className="rounded-3xl bg-white p-6 shadow-sm">
                <header className="mb-2">
                    <h3 className="text-sm font-semibold text-gray-900">Most borrowed tools</h3>
                </header>
                <div className="flex h-32 flex-col items-center justify-center rounded-2xl border border-dashed border-gray-200 bg-gray-50 text-center">
                    <p className="text-xs font-medium text-gray-600">No borrowing data yet</p>
                    <p className="mt-1 text-[11px] text-gray-500">Once users start borrowing tools, you&apos;ll see the top items here.</p>
                </div>
            </section>
        );
    }

    const sortedTools = [...tools].sort((first, second) => second.count - first.count);
    ...
    const ticks = Array.from({ length: tickSteps + 1 }, (_value, index) => Math.round(roundedMax - index * step));
    ...
    return (
        <section className="rounded-3xl bg-white p-6 shadow-sm">
            ...
            <div className="mt-4 h-52 rounded-2xl bg-slate-50 px-4 pt-4 pb-3">
                    ...
                    <div className="pointer-events-none absolute inset-x-0 top-0 bottom-0 flex flex-col justify-between text-[10px] text-slate-400">
                        {ticks.map((tick, index) => (
                            <div key={tick} className="flex items-center gap-1">
                                ...
                                <div className={index === ticks.length - 1 ? 'h-px flex-1 bg-slate-300' : 'h-px flex-1 bg-slate-200/60'} />
                            </div>
                        ))}
                    </div>

                    {/* subtle vertical grid lines inside the chart area */}
                    <div className="pointer-events-none absolute top-0 right-0 bottom-0 left-8 flex justify-between">
                        {sortedTools.map((tool) => (
                            <div key={tool.name} className="h-full w-px bg-slate-200/30" />
                        ))}
                    </div>
